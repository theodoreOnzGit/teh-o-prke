...
use uom::si::{f64::*, ratio::ratio, time::nanosecond, volumetric_number_density::per_cubic_meter, volumetric_number_rate::per_cubic_meter_second};

use std::{ops::{Deref, DerefMut}, sync::{Arc, Mutex}, time::{Instant, SystemTime}};
fn main(){
    let run_server = true;
    construct_and_run_zero_power_prke_server(run_server);
}


#[warn(missing_docs)]
pub fn construct_and_run_zero_power_prke_server(run_server: bool){
    ...
    let neutron_mean_lifetime = Time::new::<nanosecond>(100.0);
